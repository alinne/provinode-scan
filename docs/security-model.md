...
- QR payload import enforces pairing endpoint `https`, non-expired TTL, wire major version compatibility, SHA-256 fingerprint shape, Base64 HMAC-SHA256 signature payload shape (32-byte digest), and valid QUIC endpoint host:port before applying settings.
